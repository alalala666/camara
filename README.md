...
用一支普通的電腦攝影機,做即時**移動偵測**。偵測到入侵者時自動存下截圖、寄 Email 通知你(附當下畫面),並同時錄下現場的**影片**與**聲音**作為證據。
...
- 🎬 **入侵錄影**:觸發後錄下一段現場影片到 `videos/`(預設 20 秒)
- 🎤 **入侵錄音**:同時用麥克風錄下現場聲音到 `recordings/`(預設 20 秒)
- ⚡ **非阻塞設計**:寄信與錄音都在背景執行緒處理,攝影機畫面不卡頓
...
1. 📧 **立即**寄出截圖警報信
2. 🎬 錄影 20 秒 → 存到 `videos/`(不寄信)
3. 🎤 錄音 20 秒 → 存到 `recordings/`(不寄信)
...
| `--email-cooldown` | `20.0` | 兩封 Email 的最短間隔秒數，避免狂寄（同時也是錄音長度） |
| `--video-seconds` | `20.0` | 入侵時錄影的秒數 |
...
4. **警報**：存截圖 + 寄 Email（有獨立冷卻，避免重複觸發）。寄信動作丟到背景執行緒,不阻塞攝影機。
5. **錄影**：用主迴圈的畫面寫入 `videos/` 影片檔(因攝影機已被佔用,無法另開擷取)。
6. **錄音**：背景執行緒用麥克風錄一段 WAV 存到 `recordings/`,並以 `Lock` 避免重疊錄音。
...
├── videos/            # 觸發時的錄影(自動產生)
└── recordings/        # 觸發時的錄音(自動產生)
```

## 安全與隱私

- `password.txt`、`snapshots/`、`videos/`、`recordings/` 都已列入 `.gitignore`，不會被 commit。
- 帳密請務必使用 Gmail **應用程式密碼**，不要使用主帳號登入密碼。
- 截圖、錄影、錄音含個人隱私，請自行妥善保管或定期清除。
